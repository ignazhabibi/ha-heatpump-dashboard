...
export class HeatpumpInsightCard extends HeatpumpBaseCard {
    ...
    protected async _fetchData(): Promise<void> {
        ...
        try {
            ...
            if (result.yesterdayPoint) {
                datasets.push({
                    label: t.yesterday,
                    data: [result.yesterdayPoint],
            ...
            this._data = {
                ...
                yesterday: result.yesterdayMeta ? {
                    date: result.yesterdayMeta.date,
                    hdd: result.yesterdayMeta.hdd,
                    energy: result.yesterdayMeta.energy,
                    efficiency: result.yesterdayMeta.efficiency
                } : undefined
            };
        } catch (e) {
            if (requestId !== this._fetchRequestId) return;
    ...
    protected render() {
        ...
        const deviation = m.deviation ?? 0;
        const baseLoadLabel = m.baseLoadSource === 'regression' ? t.baseLoadRegression : t.baseLoad;
        const yesterdayEnergy = this._data.yesterday?.energy ?? 0;
        const expectedYesterday = yesterdayEnergy - deviation;
        const deviationPct = expectedYesterday > 0.1
            ? (deviation / expectedYesterday) * 100
            : null;
        ...
        return html`
            <ha-card>
                ...
                <div class="card-footer">
                    <div class="legend">
                        ${this._data.yesterday ? renderLegendItem(
            `${t.yesterday} (${new Date(this._data.yesterday.date).toLocaleDateString(this.hass.language || 'en', { day: '2-digit', month: '2-digit' })})`,
            '#FF9800',
            true,
            undefined,
            html`
                            <div class="yesterday-values">
                                <span class="yesterday-main">
                                    ${t.actualShort} ${yesterdayEnergy.toFixed(1)} | ${t.expectedShort} ${expectedYesterday.toFixed(1)} kWh
                                </span>
                                <span class="yesterday-dev ${deviation > 0 ? 'deviation-positive' : 'deviation-negative'}">
                                    ${deviation > 0 ? '+' : ''}${deviation.toFixed(1)} kWh${pctText}
                                </span>
                            </div>
                        `
        ) : ''}
